...
#+begin_src systemd :tangle (haris/tangle-home ".config/systemd/user/haris-background.target")
  ...
  Requires=crontab-listen-update.service,emacs.service,ssh-agent.service,clipmenud.service
  AllowIsolate=yes
...
#+end_src
* Clipmenud
NOTE: The =CM_DIR= environment variable should be in sync with the [[file:scripts.org::*=clipmenu=][clipmenu wrapper script]].
#+begin_src systemd :tangle (haris/tangle-home ".config/systemd/user/clipmenud.service")
  ...
  Environment=CM_DIR=<<eval-user-home()>>/.local/share/haris/clipmenu
  Environment=XDG_RUNTIME_DIR=<<eval-xdg-runtime-dir()>>
