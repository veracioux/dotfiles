...
#+end_src
** =flameshot=                                                       :wrapper:
#+begin_src shell :tangle ~/.local/bin/flameshot
  o --title flameshot alternative-command flameshot
  until pgrep --quiet -x "flameshot"; do
    sleep 0.1s
  done
  alternative-command flameshot "$@"
#+end_src
** =rm=                                                              :wrapper:
...
#+end_src
** =firefox=                                                         :wrapper:
#+begin_src bash :tangle ~/.local/bin/firefox
...
#+end_src
** =smart-launch=
Runs a command, and decides how to place its resulting window based on the
currently focused i3 container:
- Stacked/tabbed layout: run normally (new window tiles unobtrusively).
- Split layout: float the new window and move it to scratchpad (so tiling
  doesn't reshuffle existing windows).
- If the currently focused window is itself floating (e.g. a scratchpad), hide
  it first so the new window appears on top rather than behind it.

Usage: =smart-launch CMD...=
#+begin_src bash :tangle ~/.local/bin/smart-launch
  if [ "$#" = 0 ]; then
    echo "usage: smart-launch CMD..." >&2
    exit 2
  fi
  ...
  layout=$(echo "$tree" | jq -r \
    '.. | objects | select(.nodes[]?.focused == true) | .layout' | tail -1)

  is_floating=$(echo "$tree" | jq -r \
    '.. | objects | select(.focused == true) | .floating' | head -1)
  case "$is_floating" in
    user_on|auto_on) i3-msg move scratchpad >/dev/null ;;
  esac
  ...
  case "$layout" in
    ...
    *)
      {
        con_id=$(i3-msg -t subscribe '["window"]' \
          | jq -r --unbuffered 'select(.change == "new") | .container.id' \
          | head -1)
        i3-msg "[con_id=$con_id]" floating enable, move scratchpad, scratchpad show >/dev/null
      } &
      exec "$@"
      ;;
  esac
...
#+end_src
** =alacritty-fork=
Open a new alacritty window running the same command as the foreground process
of the current terminal. Only whitelisted commands are allowed.
#+begin_src bash :tangle ~/.local/bin/alacritty-fork
  WHITELIST=(claude opencode)

  # Walk the process tree rooted at $1 from a single ps snapshot, printing
  # each descendant's command name.
  descendants() {
      ps -e -o pid=,ppid=,comm= | awk -v root="$1" '
          { comm[$1]=$3; children[$2]=children[$2]" "$1 }
          END {
              stack=children[root]; n=0
              while (stack != "") {
                  sub(/^ /, "", stack)
                  sp=index(stack, " ")
                  if (sp) { p=substr(stack,1,sp-1); stack=substr(stack,sp+1) }
                  else    { p=stack; stack="" }
                  print comm[p]
                  if (children[p] != "") stack=children[p] stack
              }
          }'
  }

  ALACRITTY_PID=$(xdotool getactivewindow getwindowpid 2>/dev/null)
  if [ -z "$ALACRITTY_PID" ]; then
      error "alacritty-fork: could not determine active window PID"
      exit 1
  fi
  info "alacritty-fork: alacritty PID=$ALACRITTY_PID"

  # tmux daemonizes its server, so pane processes are children of the tmux
  # server (under systemd), NOT under alacritty. We find the tmux session by
  # matching alacritty's child PTY against tmux client TTYs, then walk
  # descendants from the active pane's root PID.
  CHILD_PID=$(pgrep -P "$ALACRITTY_PID" | head -1)
  CHILD_TTY=$(readlink "/proc/$CHILD_PID/fd/0" 2>/dev/null)
  info "alacritty-fork: child PID=$CHILD_PID TTY=$CHILD_TTY"

  SESSION=$(tmux list-clients -F '#{client_tty} #{session_name}' 2>/dev/null \
      | awk -v tty="$CHILD_TTY" '$1 == tty {print $2}')
  info "alacritty-fork: tmux session='$SESSION'"

  if [ -n "$SESSION" ]; then
      ROOT_PID=$(tmux display-message -t "$SESSION" -p '#{pane_pid}' 2>/dev/null)
  else
      ROOT_PID="$CHILD_PID"
  fi

  DESCENDANTS=$(descendants "$ROOT_PID")
  info "alacritty-fork: root PID=$ROOT_PID descendants=$(echo "$DESCENDANTS" | tr '\n' ' ')"

  FG_CMD=$(printf '%s\n' "$DESCENDANTS" \
      | grep -xF "$(printf '%s\n' "${WHITELIST[@]}")" \
      | head -1)

  cd "$(alacritty-pane-cwd)"
  if [ -z "$FG_CMD" ]; then
      info "alacritty-fork: no whitelisted command found, falling back to plain term"
      term &
  else
      info "alacritty-fork: forking '$FG_CMD'"
      term "$FG_CMD" &
  fi
#+end_src
** =sway=                                                          :wrapper:
...
#+end_src
** =code=                                                          :wrapper:
Runs VSCode inside a transient systemd scope under =app.slice= with a
~MemoryHigh~ ceiling and a high ~OOMScoreAdjust~ so it's the first thing killed
under memory pressure. See [[file:background.org::*Out-of-memory handling][background.org]] for the surrounding OOM setup.
#+begin_src bash :tangle ~/.local/bin/code :shebang "#!/usr/bin/env bash"
  set -e
  echo 600 > /proc/self/oom_score_adj
  exec systemd-run --user --scope --quiet \
       --unit="code-$$" \
       --slice=app.slice \
       -p MemoryHigh=12G \
       -- alternative-command code "$@"
#+end_src
** =web-ext=                                                       :wrapper:
#+begin_src bash :tangle ~/.local/bin/web-ext
  alternative-command web-ext "$@" &
  child_pid="$!"

  trap 'kill -TERM "$child_pid" 2>/dev/null' INT
...
#+end_src
** =tmux-shell=
Launches fish inside a predictably named tmux session (=term-001=, =term-002=, ...).
This enables sharing a terminal session across multiple windows and
programmatic access to the scrollback buffer via =tmux capture-pane=.
#+begin_src bash :tangle ~/.local/bin/tmux-shell
  has_newline=false
  ...
  if $has_newline; then
      echo "Warning: env contains values with newlines; falling back to plain fish" >&2
      exec fish "$@"
  fi

  counter_file="${XDG_RUNTIME_DIR:-/tmp}/tmux-shell.counter"
  {
      flock 9
  ...
  name=$(printf 'term-%03d' $n)

  launcher=$(mktemp /tmp/tmux-launch-XXXXXX)
  chmod +x "$launcher"
  {
  ...
  tmux set-option -t "$name" prefix None
  tmux set-option -t "$name" set-titles on
  tmux set-option -t "$name" set-titles-string '#{pane_title}'
  exec tmux attach -t "$name"
#+end_src
** =claude-confirm=
Scan all tmux panes for the text =requires confirmation=. On the first match,
open a new alacritty window via =term= and attach it to the session containing
that pane, with the matching pane selected.
#+begin_src bash :tangle ~/.local/bin/claude-confirm
  set -eu

  log() { printf 'claude-confirm: %s\n' "$*" >&2; }

  PATTERN='requires confirmation|Do you want to'

  panes=$(tmux list-panes -a -F '#{pane_id} #{session_name}' 2>/dev/null || true)
  if [ -z "$panes" ]; then
      log "no tmux panes found (is the server running?)"
      exit 1
  fi
  log "scanning $(printf '%s\n' "$panes" | wc -l) pane(s)"

  match=""
  while read -r pane_id session; do
      content=$(tmux capture-pane -p -J -t "$pane_id" 2>/dev/null || true)
      hit=$(printf '%s\n' "$content" | grep -E "$PATTERN" | head -n1 || true)
      if [ -n "$hit" ] && [ ! "$pane_id" = "${TMUX_PANE:-}" ]; then
          log "MATCH $pane_id (session=$session): $(printf '%.80s' "$hit")"
          match="$pane_id $session"
          break
      else
          log "skip  $pane_id (session=$session)"
      fi
  done <<<"$panes"

  if [ -z "$match" ]; then
      log "no pane awaiting confirmation"
      exit 1
  fi

  TMUX='' exec tmux attach -t "$pane_id"
#+end_src
** =claude-tip=
Print the arguments back if running inside Claude Code, no-op otherwise.
Useful for embedding hints in shell output that only Claude will see.
#+begin_src shell :tangle ~/.local/bin/claude-tip
  [ -n "$CLAUDECODE" ] && echo "Tip:" "$@"
  exit 0
#+end_src
** =rm-dottmp=
Remove the =.tmp= directory at the given path. Used to clean up scratch
directories AI agents create per the global instructions.
#+begin_src bash :tangle ~/.local/bin/rm-dottmp
  rm -r "$1/.tmp"
#+end_src
** =safe=
Run a command in a read-only sandbox using bubblewrap. The entire filesystem is
mounted read-only, with writable =/tmp= and =/run= for programs that need them.
#+begin_src shell :tangle ~/.local/bin/safe
  bwrap \
      --ro-bind / / \
      --dev /dev \
      --proc /proc \
      ...
      "$@"
  rc=$?
  [ $rc -ne 0 ] && claude-tip "if this failed due to the sandbox, retry without \`safe\`:" "$@"
  exit $rc
#+end_src
** =claude=                                                        :wrapper:
#+begin_src bash :tangle ~/.local/bin/claude
  if [ "$(pwd)" = ~ ]; then
      mkdir -p ~/.empty
      cd ~/.empty/
  fi
  # Model and effort are set globally, not per-session. Set them here to ensure
  # consistent defaults for interactive use. To use different settings, pass
  # --model or --effort explicitly to override.
  #
  # Effort/model changes from inside a Claude Code instance write to
  # ~/.claude/settings.json and would bleed into every other running instance.
  # Point CLAUDE_CONFIG_DIR at a per-session snapshot dir whose entries
  # symlink back to the real ~/.claude (and ~/.claude.json), except
  # settings.json which is a local copy — so in-session writes to it land
  # in the snapshot and are discarded on exit. Other state (projects MRU,
  # onboarding, telemetry, session files) writes through the symlinks and
  # persists normally.
  cfg=$(mktemp -d -t claude-cfg.XXXXXX)
  trap 'rm -rf "$cfg"' EXIT
  ...
  cp "$HOME/.claude/settings.json" "$cfg/settings.json"
  export CLAUDE_CONFIG_DIR="$cfg"

  # In a VM, skip permission prompts — the VM is the sandbox.
  extra=()
  if systemd-detect-virt -q; then
      extra+=(--dangerously-skip-permissions)
      # Suppress the bypass-permissions acceptance prompt by marking it accepted.
      if [ -f "$HOME/.claude.json" ] && \
         [ "$(jq 'has("bypassPermissionsModeAcceptedTwiceAt")' "$HOME/.claude.json")" = false ]; then
          tmp=$(mktemp -t claude-json.XXXXXX)
          jq --argjson ts "$(date +%s000)" \
              '.bypassPermissionsModeAcceptedTwiceAt = $ts' \
              "$HOME/.claude.json" >"$tmp" && mv "$tmp" "$HOME/.claude.json"
      fi
  fi

  exec alternative-command claude --model opus "${extra[@]}" "$@"
#+end_src
** =opencode=                                                      :wrapper:
...
* Utilities
** =alacritty-pane-cwd=
Print the current working directory of the active tmux pane in the calling
alacritty window. Falls back to =$HOME= if detection fails.
#+begin_src bash :tangle ~/.local/bin/alacritty-pane-cwd
  ALACRITTY_PID=$(xdotool getactivewindow getwindowpid 2>/dev/null)
  if [ -z "$ALACRITTY_PID" ]; then
      echo "$HOME"
      exit
  ...
  if [ -n "$SESSION" ]; then
      ...
      readlink "/proc/$CHILD_PID/cwd" 2>/dev/null || echo "$HOME"
  fi
#+end_src
** =visual-editor=
#+begin_src shell :tangle ~/.local/bin/visual-editor
  if xprop -id "$WINDOWID" | grep -q _NET_WM_STATE_FULLSCREEN || which emacsclient >/dev/null; then
      smart-launch myemacs -c --wait "$@"
  else
      editor "$@"
  fi
#+end_src
...
#+begin_src bash :tangle ~/.local/bin/adhoc
  ...
  smart-launch myemacs -c --wait "${files[@]}" >&2
...
#+begin_src shell :tangle ~/.local/bin/fifo
  ...
  session_exists() {
      env TMUX= tmux has-session -t fifo 2>/dev/null
  }

  create_session() {
      env TMUX= tmux new -s fifo "$@" tail --follow=name /tmp/fifo
  }

  if [ -t 0 ]; then
      if session_exists; then
          env TMUX= tmux attach -t fifo
      else
          create_session
      fi
  else
      if ! session_exists; then
          create_session -d || true
      fi
      # Writing to /tmp/fifo blocks until a reader (the tmux `tail` session)
      # is attached. If tmux can't start (e.g. when called from a systemd
      # user service), `tee` would hang forever — and a `timeout`-killed tee
      # may not flush stdout, so callers doing `| fifo >&2` would see empty
      # lines. So: slurp stdin, always emit to stdout first, then try the
      # fifo as a best-effort side-channel.
      input=$(cat)
      printf '%s\n' "$input"
      printf '%s\n' "$input" | timeout 1 dd of=/tmp/fifo 2>/dev/null || true
  fi
#+end_src
...
#+end_src
** =portkill=
#+begin_src bash :tangle ~/.local/bin/portkill
  set -eu
  if [ "$#" -lt 1 ]; then
  ...
  for port in "$@"; do
      out="$(lsof -nPi :"$port" || true)"
      if [ -z "$out" ]; then
          echo "No processes found on port $port" >&2
          continue
      fi
      header="$(echo "$out" | head -n 1)"
      echo "$out" | tail -n +2 | grep LISTEN | while IFS= read -r line; do
          ...
          read -r ans </dev/tty
          case "$ans" in
              y|Y|yes|YES) kill "$pid" && echo "Killed $pid" ;;
              *) echo "Skipped $pid" ;;
          esac
      done
  done
...
#+begin_src bash :tangle ~/.local/bin/info
  if [ -t 2 ]; then echo -ne "\033[1;32m" >&2; fi
  echo "$@" | fifo >&2
  if [ -t 2 ]; then echo -ne "\033[0m" >&2; fi
#+end_src
** =debug=
#+begin_src bash :tangle ~/.local/bin/debug
  if [ -t 2 ]; then echo -ne "\033[1;33m" >&2; fi
  bash -c 'printf "%q " "$@"' -s "$@" >&2
  echo | fifo >&2
  if [ -t 2 ]; then echo -ne "\033[0m" >&2; fi
...
#+end_src
** =warn=
#+begin_src bash :tangle ~/.local/bin/warn
  if [ -t 2 ]; then echo -ne "\033[1;33m" >&2; fi
  echo "$@" | fifo >&2
  if [ -t 2 ]; then echo -ne "\033[0m" >&2; fi
#+end_src
** =error=
#+begin_src bash :tangle ~/.local/bin/error
  if [ -t 2 ]; then echo -ne "\033[1;31m" >&2; fi
  echo "$@" | fifo >&2
  if [ -t 2 ]; then echo -ne "\033[0m" >&2; fi
...
#+begin_src bash :tangle ~/.local/bin/git-wt-for
  # Usage: git wt-for <command> [args...]
  set -e
  if [ $# -eq 0 ]; then
      echo "Usage: git wt-for <command> [args...]" >&2
      exit 1
  fi
  status=0
...
Utility scripts for AI agents to use.
*** =git-commit-from-editmsg=
#+begin_src shell :tangle ~/.local/bin/git-commit-from-editmsg
  git commit --file "$(git rev-parse --git-dir)"/COMMIT_EDITMSG
#+end_src
*** =git-dotharis-commit=
Thin wrapper around =git commit= used by the =/commit= slash command inside the
dotfiles repo. Forces the worktree to =~/.haris= regardless of the caller's
CWD, so an AI agent operating in a subdirectory or a different worktree still
commits against the dotfiles repo. Exists as a distinct command so the
permission system can allow it without opening up =git commit *= globally.
#+begin_src shell :tangle ~/.local/bin/git-dotharis-commit
  exec git -C ~/.haris commit "$@"
#+end_src
* Applications
...
#+end_src
** =eman=
#+begin_src shell :tangle ~/.local/bin/eman
  if [ -n "$1" ]; then
      smart-launch myemacs -c --eval "(man \"$1\")"
  else
      smart-launch myemacs -c --eval "(call-interactively 'man)"
  fi
#+end_src
#+CALL: eman(argv='())
#+CALL: eman(argv='("vim"))
...
#+end_src
** =edocker=
#+begin_src shell :tangle ~/.local/bin/edocker
  smart-launch myemacs -c --eval "(run-with-timer 0.3 nil 'docker)" "$@"
#+end_src
** =edocker-compose=
#+begin_src shell :tangle ~/.local/bin/edocker-compose
  smart-launch myemacs -c --eval "(run-with-timer 0.3 nil 'docker-compose)" "$@"
#+end_src
** =ebluetooth=
#+begin_src shell :tangle ~/.local/bin/ebluetooth
  smart-launch myemacs -c --eval '(bluetooth-list-devices)'
#+end_src
** =proced=
#+begin_src shell :tangle ~/.local/bin/proced
  <<add-create-frame-option-if-term-dumb>>
  smart-launch myemacs -c --eval '(progn (proced) (delete-other-windows))' "$@"
#+end_src
** =magit=
#+begin_src shell :tangle ~/.local/bin/magit
  smart-launch myemacs -c --eval "(magit-status)" "$@"
#+end_src
** =myemacs-here=
Open emacs in the current tmux pane's working directory.
#+begin_src shell :tangle ~/.local/bin/myemacs-here
  cd "$(alacritty-pane-cwd)"
  smart-launch myemacs -c .
#+end_src
** =qr=
...
#+begin_src bash :tangle ~/.local/bin/kbind
  ...
  pkill -f '^sxhkd\.tmp '
  smart-launch myemacs --wait -c ~/.tmp.sxhkdrc
...
#+begin_src bash :tangle ~/.local/bin/lock-screen
  ...
  picom_running="$(systemctl --user is-active -q picom.service && echo true)"
...
#+begin_src bash :tangle ~/.local/lib/dmenu/todo
  ...
  if [ "$choice" = " Edit..." ]; then
      ...
      smart-launch myemacs -c "$(echo "$lookup" | grep "/$choice/TODO.org")"
  fi
#+end_src
** Pacman
#+begin_src shell :tangle ~/.local/lib/dmenu/pacman
  ...
  case "$choice" in
      ...
      "$remove")
          ...
          ;;
      "$manage")
          smart-launch myemacs -c "$(fcmd system-install)"
          ;;
      "$keyring")
...
#+begin_src python :tangle ~/.local/lib/dmenu/config
  ...
  if choice == 'dmenu':
      EVAL = fr'''
          (progn (find-file "~/.haris/scripts.org")
                 ...
                 (evil-ex-nohiglight))
      '''
      run(["smart-launch", "myemacs", "-c", "--eval", EVAL])
  if choice.startswith('dmenu-'):
      ...
      run(["alacritty-float", "-e", "fish", "-C", "cd ~/.haris && claude"])
...
#+begin_src bash :tangle ~/.local/bin/term
  alacritty --working-directory "$PWD" -e tmux-shell -C "$([ "$#" -ge 1 ] && printf %q\  "$@" || :)"
#+end_src
** =worktree-workspace=
Set up an i3 workspace for working on a worktree (or any picked directory):
fzf-pick a dir via the fish =a= function, then arrange the workspace as
=split-h[ stacked[term, magit], stacked[claude] ]=. Aborts if the current
workspace is not empty.

The picker→orchestrator handoff goes through a single-shot named fifo: fish
writes either the chosen =$PWD= or =CANCELLED= back, and the orchestrator blocks
on =cat= until it gets one of them.
#+begin_src bash :tangle ~/.local/bin/worktree-workspace
  set -u

  abort() {
      notify-send -u critical 'worktree-workspace' "$1"
      exit 1
  }

  # Snapshot of all leaf window con-ids on the focused workspace.
  ws_window_ids() {
      i3-msg -t get_tree | jq -c --arg n "$ws_name" '
          [.. | objects | select(.type=="workspace" and .name==$n)] | .[0]
          | [.. | objects | select(.window?) | .id]'
  }

  # Run "$@" in the background and block until a new window appears on the
  # current workspace. Polls the i3 tree because i3-msg -t subscribe block-
  # buffers its ack, which makes event-driven sync unreliable.
  launch_and_wait() {
      local before id="" i
      before=$(ws_window_ids)
      ( "$@" ) &
      for i in $(seq 1 150); do
          sleep 0.1
          id=$(i3-msg -t get_tree | jq -r --arg n "$ws_name" --argjson b "$before" '
              [.. | objects | select(.type=="workspace" and .name==$n)] | .[0]
              | [.. | objects | select(.window?) | .id] - $b | .[0] // empty')
          [ -n "$id" ] && break
      done
      [ -n "$id" ] || abort 'timed out waiting for new window'
  }

  # 1. Empty-workspace check
  ws_name=$(i3-msg -t get_workspaces | jq -r '.[] | select(.focused) | .name')
  [ -n "$ws_name" ] || abort 'no focused workspace'
  count=$(i3-msg -t get_tree | jq --arg n "$ws_name" '
      [.. | objects | select(.type=="workspace" and .name==$n)] | .[0]
      | (.nodes + .floating_nodes) | length')
  [ "$count" = "0" ] || abort "workspace '$ws_name' is not empty"

  # 2. Single-shot fifo for picker→orchestrator handoff, plus a fish snippet
  #    file (term's printf-%q quoting mangles multi-line args fed to fish, so
  #    we pass two plain args — `source <path>` — and let fish read the file)
  fifo=$(mktemp -ut wt-ws.XXXXXX)
  mkfifo "$fifo"
  snippet=$(mktemp -t wt-ws.XXXXXX.fish)
  trap 'rm -f "$fifo" "$snippet"' EXIT

  # 3. Open terminal; fish runs `a`, writes the chosen $PWD (or CANCELLED) back
  cat > "$snippet" <<EOF
  set -l fifo '$fifo'
  if a
      echo \$PWD > \$fifo
  ...
  # 4. Block for the dir; empty payload = terminal closed before write
  dir=$(cat "$fifo")
  case "$dir" in
      CANCELLED|"") abort 'cancelled' ;;
      /*) [ -d "$dir" ] || abort "not a directory: $dir" ;;
      *)  abort "unexpected payload: $dir" ;;
  esac

  # 5. Wrap term in its own container and stack it
  i3-msg split v >/dev/null
  i3-msg layout stacked >/dev/null

  # 6. Magit joins the stacked container (focus is still on term → its parent)
  launch_and_wait sh -c \
      'cd "$1" && smart-launch myemacs -c --eval "(magit-status \"$1\")"' _ "$dir"

  # 7. Escape the stacked container: focus magit → its parent (the stacked
  #    container) → its parent (workspace root). Force horizontal split so the
  #    next window lands to the right rather than below.
  i3-msg focus parent >/dev/null
  i3-msg focus parent >/dev/null
  i3-msg split horizontal >/dev/null

  # 8. Claude opens as a sibling of the stacked container at workspace root
  launch_and_wait sh -c 'cd "$1" && term claude' _ "$dir"

  # 9. Wrap claude in its own stacked container so further windows can stack
  i3-msg split v >/dev/null
  i3-msg layout stacked >/dev/null
#+end_src
*** Dependencies                                                     :deps:
#+begin_src text :tangle (haris/tangle-deps "worktree-workspace.pacman")
  jq
  i3-wm
#+end_src
** =aurvote=
