...
#+begin_src bash :tangle ~/.local/bin/alacritty-fork
  ...
  # Walk the process tree rooted at $1, printing each descendant's command name
  descendants() {
      local pid=$1
      for child in $(pgrep -P "$pid" 2>/dev/null); do
          ps -p "$child" -o comm= 2>/dev/null
          descendants "$child"
      done
  }
  ...
  info "alacritty-fork: root PID=$ROOT_PID descendants=$(descendants "$ROOT_PID" | tr '\n' ' ')"

  FG_CMD=$(descendants "$ROOT_PID" \
      | grep -xF "$(printf '%s\n' "${WHITELIST[@]}")" \
      | head -1)
...
#+begin_src shell :tangle ~/.local/bin/claude
  ...
  exec alternative-command claude --model opus "$@" --effort low
#+end_src
** =opencode=                                                      :wrapper:
