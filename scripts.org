...
Launches =myemacs= normally if the current i3 container layout is split
(horizontal/vertical), but as =myemacs-float= if the layout is stacking or
tabbed.
...
#+begin_src elisp :tangle ~/.local/bin/eman
  (with-selected-frame (make-frame `((display . ,(getenv "DISPLAY"))
...
#+end_src
** =i3-float-focused=
Float the currently focused window via i3-msg (elisp). Used by elisp scripts
that create Emacs frames and want them to float without relying on i3 window
rules.
#+NAME: i3-float-focused
#+begin_src elisp :tangle no
  (start-process "i3-float" nil "i3-msg"
                 "floating enable, move scratchpad, scratchpad show")
#+end_src
** =eval-user-name=
#+NAME: eval-user-name
#+begin_src elisp
