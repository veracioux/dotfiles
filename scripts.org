...
#+begin_src bash :tangle ~/.local/bin/myemacs-float
  {
    i3-msg -t subscribe '["window"]' \
      ...
      | head -1 > /dev/null
    i3-msg floating enable, move scratchpad, scratchpad show
  } &
  myemacs -c "$@"
#+end_src
...
#+end_src
** =tmux-shell=
Launches fish inside a predictably named tmux session (=term-0=, =term-1=, ...).
...
#+begin_src bash :tangle ~/.local/bin/tmux-shell
  ...
  n=0
  while tmux has-session -t "term-$n" 2>/dev/null; do
      n=$((n + 1))
  done
  ...
  tmux new-session -d -s "term-$n" "$launcher"
  tmux set-option -t "term-$n" prefix None
  tmux set-option -t "term-$n" set-titles on
  tmux set-option -t "term-$n" set-titles-string '#{pane_title}'
  exec tmux attach -t "term-$n"
#+end_src
** =safe=
