...
#+begin_src bash :tangle ~/.local/bin/myemacs-float
  {
  ...
  myemacs -c "$@"
#+end_src
** =myemacs-smart=                                                 :wrapper:
Launches =myemacs= normally if the current i3 container layout is stacking or
tabbed (where a new tiled window is unobtrusive), but as =myemacs-float= in
split layouts (where tiling would reshuffle windows).
#+begin_src bash :tangle ~/.local/bin/myemacs-smart
  tree=$(i3-msg -t get_tree)

  layout=$(echo "$tree" | jq -r \
    '.. | objects | select(.nodes[]?.focused == true) | .layout' | tail -1)

  # If the focused window is floating (e.g. scratchpad), hide it first so the
  # new window appears on top rather than behind it
...
#+begin_src shell :tangle ~/.local/bin/eman
  if [ -n "$1" ]; then
      myemacs-float --eval "(man \"$1\")"
  else
      myemacs-float --eval "(call-interactively 'man)"
  fi
#+end_src
#+CALL: eman(argv='())
#+CALL: eman(argv='("vim"))
...
#+begin_src shell :tangle ~/.local/bin/edocker
  myemacs-float --eval "(run-with-timer 0.3 nil 'docker)" "$@"
#+end_src
** =edocker-compose=
#+begin_src shell :tangle ~/.local/bin/edocker-compose
  myemacs-float --eval "(run-with-timer 0.3 nil 'docker-compose)" "$@"
#+end_src
** =ebluetooth=
...
#+begin_src shell :tangle ~/.local/bin/magit
  myemacs-smart --eval "(magit-status)" "$@"
#+end_src
** =myemacs-here=
