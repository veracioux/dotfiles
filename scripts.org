...
#+begin_src bash :tangle ~/.local/bin/git
  ...
  claude-tip "you only need -C to act on a git worktree not reachable from your cwd; otherwise omit -C"
...
#+begin_src bash :tangle ~/.local/bin/claude
  ...
  exec alternative-command claude --model opus "$@"
#+end_src
** =opencode=                                                      :wrapper:
