...
- =.ubuntu= :: APT packages that should be installed on Ubuntu 22.04, using =sudo
  apt install=. I might subdivide this for multiple Ubuntu versions.
...
* System setup
To set up a new system, you can look at [[system-setup.org]]. It is incomplete, but
...
** Ignored files
*** rg
#+begin_src text :tangle ~/.rgignore
