...
#+begin_src sxhkdrc :tangle ~/.config/sxhkd/sxhkdrc
  ...
  mod4 + shift + v
      vscode
  mod4 + v
      smart-launch myemacs -c --eval '(scratch-buffer)'
  mod4 + shift + e
...
#+begin_src i3wm-config
  ...
  bindsym 2               move workspace to output DP-1
...
#+NAME: org-menu
#+begin_src i3wm-config
  set $mode_org        " ORG [t]odo [k]knowledge [j]ournal [n]otes [c]omputers [l]ifestyle [T]odos con[v]ersations [p]actos [a]ll"

  mode $mode_org {
  bindsym t               exec --no-startup-id "smart-launch myemacs -c ~/wiki/todo.org", mode default
  bindsym k               exec --no-startup-id "smart-launch myemacs -c ~/wiki/knowledge.org", mode default
  bindsym j               exec --no-startup-id "smart-launch myemacs -c ~/wiki/journal.org", mode default
  bindsym n               exec --no-startup-id "smart-launch myemacs -c ~/wiki/notes.org", mode default
  bindsym c               exec --no-startup-id "smart-launch myemacs -c ~/wiki/computers.org", mode default
  bindsym l               exec --no-startup-id "smart-launch myemacs -c ~/wiki/lifestyle.org", mode default
  bindsym Shift+t         exec --no-startup-id "dmenu_run todo", mode default
  bindsym p               exec --no-startup-id "smart-launch myemacs -c ~/wiki/pactos.org", mode default
  bindsym a               exec --no-startup-id "smart-launch myemacs -c ~/wiki/index.org", mode default
...
#+begin_src i3wm-config
  ...
  mode $mode_notif {
  bindsym a       exec --no-startup-id <<scripts-dir>>/notification-open-url.sh, mode default
...
Closes the currently displayed notification (so it lands in history), extracts
the first URL from its summary/body, and opens it with =xdg-open=. Handles both
plain =https://...= text and pango =<a href="...">= markup.
#+begin_src bash :tangle ~/.config/i3/scripts/notification-open-url.sh :tangle-mode (eval #o744) :shebang "#!/usr/bin/env bash"
  set -eu

  dunstctl close 2>/dev/null || true

  content=$(dunstctl history | jq -r '
    .data[0][0] // empty | (.summary.data // "") + " " + (.body.data // "")
  ')

  url=$(echo "$content" | grep -oE 'href="[^"]+"' | head -1 | sed 's/href="//; s/"$//')
  [ -z "$url" ] && url=$(echo "$content" | grep -oE 'https?://[^ "<>]+' | head -1)
...
#+begin_src i3wm-config
  set $mode_chat       " Chat | Reasoning level [1] [2] [3] [4] [5] [c]hoose | [⏎] claude-confirm"
  ...
  bindsym Return  exec --no-startup-id smart-launch term claude-confirm, mode default
  bindsym Escape  mode default
  }
...
#+begin_src i3wm-config
  set $mode_extra_ws "Extra WS | [a-f] [1-9] | Shift+key to move | [n]ext | [w]orktree-ws"
  ...
  bindsym 1               workspace $ws_n1, mode default
  bindsym 2               workspace $ws_n2, mode default
  bindsym 3               workspace $ws_n3, mode default
  bindsym 4               workspace $ws_n4, mode default
  bindsym 5               workspace $ws_n5, mode default
  bindsym 6               workspace $ws_n6, mode default
  bindsym 7               workspace $ws_n7, mode default
  bindsym 8               workspace $ws_n8, mode default
  bindsym 9               workspace $ws_n9, mode default

  bindsym Shift+1         move container to workspace $ws_n1, mode default
  bindsym Shift+2         move container to workspace $ws_n2, mode default
  bindsym Shift+3         move container to workspace $ws_n3, mode default
  bindsym Shift+4         move container to workspace $ws_n4, mode default
  bindsym Shift+5         move container to workspace $ws_n5, mode default
  bindsym Shift+6         move container to workspace $ws_n6, mode default
  bindsym Shift+7         move container to workspace $ws_n7, mode default
  bindsym Shift+8         move container to workspace $ws_n8, mode default
  bindsym Shift+9         move container to workspace $ws_n9, mode default

  bindsym n               exec --no-startup-id <<scripts-dir>>/aux-ws-next.sh, mode default
  bindsym w               exec --no-startup-id worktree-workspace, mode default
...
#+end_src
*** Cycle to next aux workspace
Cycles through existing =aux-*= workspaces in order: letters first
(alphabetical), then digits (numerical). Only visits workspaces that currently
exist (i.e. have at least one window or are focused). Wraps around. Robust to
future additions of new letters/digits — no hardcoded cycle list.
#+begin_src bash :tangle ~/.config/i3/scripts/aux-ws-next.sh :tangle-mode (eval #o744) :shebang "#!/usr/bin/env bash"
  set -eu

  ws_json=$(i3-msg -t get_workspaces)

  mapfile -t letters < <(echo "$ws_json" | jq -r '.[].name' \
    | sed -n 's/.*:aux-\([a-z]\+\)$/\1/p' | sort)
  mapfile -t numbers < <(echo "$ws_json" | jq -r '.[].name' \
    | sed -n 's/.*:aux-\([0-9]\+\)$/\1/p' | sort -n)

  cycle=("${letters[@]}" "${numbers[@]}")
  [ ${#cycle[@]} -eq 0 ] && exit 0

  current=$(echo "$ws_json" \
  ...
  full=$(echo "$ws_json" \
    | jq -r --arg s ":aux-$target" '.[].name | select(endswith($s))' \
    | head -1)
  i3-msg "workspace \"$full\""
#+end_src
...
#+end_src
** Auto-show on menu mode
When any i3 mode (menu) is active, dock the bar; restore =hide= on return to
=default=. Implemented as an IPC subscriber so individual menu bindings don't
need to change. Lifecycle is managed by =i3-bar-mode-on-menu.service= in
[[file:background.org::*Bar mode on menu][background.org]], pulled in by =haris-gui-i3.target=.
#+begin_src bash :tangle ~/.config/i3/scripts/bar-mode-on-menu.sh :tangle-mode (eval #o744) :shebang "#!/usr/bin/env bash"
  set -eu
  ...
  info "bar-mode-on-menu: started pid=$$ bar_id=$bar_id"

  i3-msg -t subscribe -m '["mode"]' | while read -r line; do
      mode=$(echo "$line" | jq -r '.change')
      if [ "$mode" = "default" ]; then
          # Restore whatever the bar was set to before we entered menu mode.
          # Submenus keep `in_menu=true`, so saved_mode is captured once at the
          # entry from default and not overwritten by intermediate transitions.
          out=$(i3-msg "bar mode $saved_mode" 2>&1) || true
          in_menu=false
          info "bar-mode-on-menu: mode=default -> bar $saved_mode | $out"
      else
          if ! $in_menu; then
              saved_mode=$(i3-msg -t get_bar_config "$bar_id" | jq -r '.mode')
              in_menu=true
          fi
          out=$(i3-msg 'bar mode dock' 2>&1) || true
          info "bar-mode-on-menu: mode=$mode (saved=$saved_mode) -> bar dock | $out"
      fi
  done
#+end_src
** i3status
...
#+begin_src python :tangle ~/.config/i3status/custom-script.py :tangle-mode (identity #o744)
  ...
  def get_mic_volume_widget():
      ...
      match = re.search(r'(\d+\.?\d*%)', volume_output)
      if not match:
          return None
      volume = match[1]
...
#+begin_src i3wm-config
  ...
  for_window [instance="dragon-drop"]                   sticky toggle
  ...
  # Send Playwright browser windows to extra workspace
  for_window [instance=".*playwright.*"] floating enable
  assign [instance=".*playwright.*"] number $ws6
#+end_src
* Compositor
:PROPERTIES:
