  ...
  (run-with-timer
   ...
   (lambda ()
     (let ((default-directory "~/"))
       (call-process "fish" nil 0 nil "-c" "sxhkd & disown")
       (if (locate-file exec-path "sxhkd.private")
           (call-process "fish" nil 0 nil "-c" "sxhkd.private & disown")))))
#+end_src
...
#+begin_src shell :tangle ~/.xinitrc-i3
  ...
  export TERMINAL=alacritty                   # Used by i3-sensible-terminal
...
** Applications menu
#+begin_src i3wm-config
  set $mode_apps "Apps | [p]acman [c]lipmenu [s]ecclip [e]lement [v]iber [u]nicode [V]pn [t]eams"
...
#+begin_src i3wm-config
  ...
  for_window [class="Google-chrome" instance="^google-chrome \\(/tmp/\\.org\\.chromium\\.Chromium"] floating enable
  assign [class="Google-chrome" instance="^google-chrome \\(/tmp/\\.org\\.chromium\\.Chromium"]     number $ws6
#+end_src
* Compositor
:PROPERTIES:
