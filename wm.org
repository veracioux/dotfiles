...
#+begin_src i3wm-config
  set $mode_chat       " Chat | ChatGPT [c]hat [C]hoose"

  mode $mode_chat {
  bindsym c       exec --no-startup-id <<scripts-dir>>/chatgpt.sh, mode default
  bindsym Shift+c exec --no-startup-id <<scripts-dir>>/chatgpt.sh --choose, mode default
  bindsym Escape  mode default
  }
