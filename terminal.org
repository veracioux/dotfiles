...
** General
#+begin_src toml
  terminal.shell.program = "tmux-shell"
...
#+begin_src toml
  ...
  bindings = [
    ...
    { key = "T", mods = "Alt", command = { program = "sh", args = ["-c", "cd \"$(alacritty-pane-cwd)\" && alacritty"] } },
    { key = "G", mods = "Alt|Shift", command = { program = "sh", args = ["-c", "tmux new -d -c \"$(alacritty-pane-cwd)\" magit"] } },
    { key = "P", mods = "Alt|Shift", command = { program = "sh", args = ["-c", "passmenu -l 10 && xdotool key Ctrl+Shift+V Return"] }, mode = "~AppCursor" },
    { key = "E", mods = "Alt|Shift", command = { program = "sh", args = ["-c", "tmux new -d myemacs-here"] } },
    { key = "F9", mods = "Alt", chars = "\\e[20~" },
    { key = "P", mods = "Control", chars = "\u001b[A" },
    { key = "N", mods = "Control", chars = "\u001b[B" },
    { key = "P", mods = "Alt", chars = "\u001b[A" },
    { key = "N", mods = "Alt", chars = "\u001b[B" },
    ...
    { key = "V", mods = "Alt", command = { program = "sh", args = ["-c", "smart-launch code \"$(alacritty-pane-cwd)\""] } },
    { key = "Enter", mods = "Shift", chars = "\u001b[27;2;13~", mode = "~Search" },
    { key = "F", mods = "Alt", command = { program = "alacritty-fork" } },
  ]
#+end_src
* Tuterm
#+begin_src shell :tangle ~/.config/tuterm/config.sh
  TUTERM_SHELL='fish'
...
#+end_src
* Tmux
#+begin_src shell :tangle ~/.tmux.conf :shebang ""
  set -g status off
  ...
  # Open a fresh window pointed at the next Claude pane awaiting confirmation
  bind-key -n M-C run-shell "term claude-confirm"
  ...
  # Customize `tmux ls` output (tab-aligned): name, windows, created, attached, active pane id.
  # Commas inside the format are escaped (\,) because `set -a` on array options splits values on comma.
  # Fixed index keeps reloads idempotent.
  set -s command-alias[100] 'ls=list-sessions -F "#{session_name}\t#{session_windows} windows\t(created #{t:session_created})\t#{pane_id}\t#{?session_attached\,(attached)\,}"'
#+end_src
* Screen
...
#+end_src
* Cheat
#+begin_src yaml :tangle ~/.config/cheat/conf.yml
  colorize: true
  style: monokai
  pager: less -FRX
  cheatpaths:
...
#+end_src
* Tem
#+begin_src shell :tangle ~/.config/tem/config
  [general]
  repo_path =
  editor = myemacs -c
...
* Vifm
:PROPERTIES:
:header-args+: :tangle ~/.config/vifm/vifmrc
:END:
#+begin_src vimrc :tangle ~/.config/vifm/vifmrc
  " vim: filetype=vifm
...
#+end_src
* Dialog
#+begin_src python :tangle ~/.dialogrc
  aspect                      = 0
  separate_widget             = ""
  tab_len                     = 1
  visit_items                 = OFF
...
* Translate shell
** Main
#+begin_src lisp :tangle ~/.config/translate-shell/init.trans
  {
  :theme "custom.trans"
  }
#+end_src
** Color scheme
#+begin_src lisp :tangle ~/.config/translate-shell/custom.trans
  {
...
* Cointop
:PROPERTIES:
:header-args+: :tangle ~/.config/cointop/config.toml
:END:
#+begin_src toml
  currency = "USD"
...
#+begin_src systemd :tangle ~/.config/systemd/user/cointop.service
  [Unit]
...
#+end_src
* Ticker
#+begin_src yaml :tangle ~/.ticker.yml
  # ~/.ticker.yaml
  show-summary: true
  show-tags: true
...
#+begin_src yaml :tangle ~/.config/tickrs/config.yml
  symbols:
    - SPY
    - AMD
    - NET
