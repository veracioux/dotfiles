...
#+begin_src fish
  ...
  # SSH doesn't forward COLORTERM, so truecolor-aware tools (delta, bat, ...)
  # fall back to the 16-color palette. Restore it.
...
=~/.profile= is loaded eagerly at startup if =HARIS_PROFILE_LOADED= isn't already set
(i.e. the wrapping shell didn't source it for us).
#+begin_src fish
  if type --quiet bass; and [ -z "$HARIS_PROFILE_LOADED" ]
      bass source ~/.profile
  end
...
#+begin_src fish :tangle ~/.config/fish/functions/fish_prompt.fish
  function fish_prompt --description 'Write out the prompt'
      ...
      echo -n -s  (set_color --bold brblue)   "$USER$host "               \
                  ...
                  (set_color white)           "$tmux_pane"        $normal \
                  \n                                             \
                  (set_color green)           $node_env                   \
                  " " (__haris_prompt_compose_status)                     \
                  "$(__haris_prompt_env_vars)"                            \
                  (set_color yellow)          \n" $suffix "
...
***** Helpers
****** =__haris_prompt_env_vars=
#+begin_src fish :tangle ~/.config/fish/functions/__haris_prompt_env_vars.fish
  function __haris_prompt_env_vars --argument-names cmd
      argparse h/help -- $argv
      if [ -n "$_flag_help" ]
          echo "Usage: __haris_prompt_env_vars [dump]"
          echo
          echo "Commands:"
          echo "  dump    Dump the current variable values as the baseline"
          return
      end
      set -l file ~/.local/state/haris_prompt_env_defaults.txt
      mkdir -p ~/.local/state/
      if [ "$cmd" = "dump" ] || [ ! -f "$file" ]
          env > "$file"
          return
      end
      if env | grep -q '^VSCODE_'
          echo (set_color --bold yellow)" VSCODE"
          return
      end
      awk '
          BEGIN { FS="=" }
          NR==FNR { defaults[$1] = substr($0, index($0, "=") + 1); next }
          { runtime[$1] = substr($0, index($0, "=") + 1) }
          END {
              for (var in defaults) {
                  if (!(var in runtime)) {
                      print var, "|", defaults[var], "| "
                  } else {
                      print var, "|", defaults[var], "|", runtime[var]
                  }
              }
              for (var in runtime) {
                  if (!(var in defaults)) {
                      print var, "|  |", runtime[var]
                  }
              }
          }
      ' "$file" (env | psub) | while read -l line
          set -l var (string split -f1 " | " -- "$line")
          set -l default_value (string split -f2 " | " -- "$line")
          set -l runtime_value (string split -f3 " | " -- "$line")
          switch "$var"
              case \
                  ALACRITTY_LOG \
                  ALACRITTY_SOCKET \
                  ...
                  XDG_VTNR \
                  ''
                  # NOTE: For some reason vscode gets the *SECURELOCK* vars
                  continue
              case NODE_ENV
                  if [ -z "$runtime_value" ]
                      continue
                  end
                  set_color --bold green && echo -n "  $runtime_value" && set_color normal
                  continue
              case BUN_OPTIONS NODE_OPTIONS
                  if [ -z "$default_value" ] && [ -n "$runtime_value" ]
                      set_color --bold yellow
                      echo -n " $var=$runtime_value"
                      continue
                  end
          end

          # Display removed variables
          if [ -n "$default_value" ] && [ -z "$runtime_value" ]
              set_color red
              echo -n " -$var"
              set_color normal
              continue
          ...
          if [ "$default_value" != "$runtime_value" ]
              set_color --bold yellow
              echo -n " $var"
              set_color normal
              continue
...
#+end_src
****** =__haris_prompt_compose_status=
#+begin_src fish :tangle ~/.config/fish/functions/__haris_prompt_compose_status.fish
...
#+end_src
*** NVM version auto-switch
#+begin_src fish
  if type -q nvm
      function _ensure_nvm --on-event fish_postexec
          set -l dir "$PWD"
          if [ "$PWD" != "$__haris_nvm_dir" ]
              nvm use 2>/dev/null
              set -g __haris_nvm_dir "$PWD"
          end
      end
  end
#+end_src
*** =edit_command_buffer_custom=
#+begin_src fish :tangle ~/.config/fish/functions/edit_command_buffer_custom.fish
  set -l actual_editor
...
** Bindings
:PROPERTIES:
:header-args:fish+: :tangle ~/.config/fish/functions/fish_user_key_bindings.fish
:END:
#+begin_src fish :tangle ~/.config/fish/functions/bind_all_modes.fish
  function bind_all_modes
      for mode in default insert replace replace_one visual
          bind -M "$mode" $argv
      end
  end
#+end_src
...
#+begin_src fish
  ...
  bind_all_modes \ec '__haris_copy_or_capture'
  ...
  bind_all_modes \eE 'smart-launch myemacs -c .'
...
#+end_src
**** Helper functions
***** =__haris_copy_or_capture=
Copy command line to clipboard, or if empty, copy the tmux pane contents.
#+begin_src fish :tangle ~/.config/fish/functions/__haris_copy_or_capture.fish
  function __haris_copy_or_capture
      set -l cmd (string trim (commandline -b))
      if test -z "$cmd"
          echo -n "$TMUX_PANE" | xsel -b -i
          notify-send "Copied $TMUX_PANE to clipboard"
      else
          fish_clipboard_copy
...
#+end_src
***** =__haris_run_in_bash=
#+begin_src fish :tangle ~/.config/fish/functions/__haris_run_in_bash.fish
...
#+begin_src fish :tangle ~/.config/fish/functions/__haris_cmdline_prepend.fish
...
#+end_src
***** =__haris_show_man=
#+begin_src fish :tangle ~/.config/fish/functions/__haris_show_man.fish
  function __haris_show_man
      eman (commandline --current-process --tokenize | grep -v '^sudo$' | head -1)
  end
#+end_src
***** =__haris_cmdline_append_or_toggle=
#+begin_src fish :tangle ~/.config/fish/functions/__haris_cmdline_append_or_toggle.fish
...
#+end_src
***** =__haris_test_eval=
#+begin_src fish :tangle ~/.config/fish/functions/__haris_test_eval.fish
  function __haris_test_eval
      eval "$(adhoc - (mktemp -t XXXXXXXX.fish) 2>/dev/null)"
  end
#+end_src
***** =__haris_copy_cmdline_and_output=
#+begin_src fish :tangle ~/.config/fish/functions/__haris_copy_cmdline_and_output.fish
...
** Functions
*** =whatis=
#+begin_src fish :tangle ~/.config/fish/functions/whatis.fish
...
#+end_src
*** =ndir=
#+begin_src fish :tangle ~/.config/fish/functions/ndir.fish
  # Create a new dir and cd
  function ndir;  mkdir -p "$argv"; cd "$argv"; end
#+end_src
*** =1=
#+begin_src fish :tangle ~/.config/fish/functions/1.fish
  # Print first argument
  function 1; echo $argv[1]; end
#+end_src
*** =a=
#+begin_src fish :tangle ~/.config/fish/functions/a.fish
  # Run z through fzf, augmented with git worktree and pnpm workspace entries
  function a
      # z entries (score + path)
      z -l $argv | read -z choices
      set -l paths (echo "$choices" | sed '/^$/d' | tac | sed -E 's/^\S+\s+//')

      # Add worktree and pnpm entries if inside a git repo
      if set -l git_root (git rev-parse --show-toplevel 2>/dev/null)
          set -l rel (realpath "$PWD" --relative-to "$git_root")

          # Worktree entries: each worktree root + current relative subpath
          for wt in (git worktree list --porcelain 2>/dev/null | grep '^worktree ' | sed 's/^worktree //')
              set -l candidate "$wt/$rel"
              test -d "$candidate"; and set -a paths "$candidate"
          end

          # pnpm workspace entries (cached for 20 min per git root)
          if type -q pnpm; and test -f "$git_root/pnpm-workspace.yaml"
              set -l cache_file ~/.cache/pnpm-workspaces-(echo "$git_root" | string replace -a / _)
              if not test -f "$cache_file"; or test (math (date +%s) - (stat -c %Y "$cache_file")) -gt 1200
                  pnpm ls -r --depth -1 --long --parseable 2>/dev/null | cut -d: -f1 > "$cache_file"
              end
              set -a paths (cat "$cache_file")
          end
      end

      # Dedupe and select
      set -l unique_paths (printf '%s\n' $paths | awk '!seen[$0]++')
      set dest (printf '%s\n' $unique_paths | fzf --select-1)

      test -n "$dest"; and cd "$dest"
  end
#+end_src
**** Completions
#+begin_src fish :tangle ~/.config/fish/completions/a.fish
  complete -c a --no-files --wraps z
#+end_src
*** =open=
#+begin_src fish :tangle ~/.config/fish/functions/open.fish
  # Open a GUI app and disown
  function open; for file in $argv; o xdg-open "$file"; end; end
#+end_src
*** =imount=
#+begin_src fish :tangle ~/.config/fish/functions/imount.fish
...
#+end_src
*** =vh=
#+begin_src fish :tangle ~/.config/fish/functions/vh.fish
  # Vim help
  function vh; vim -c ":h $argv | only"; end
#+end_src
*** =copypath=
#+begin_src fish :tangle ~/.config/fish/functions/copypath.fish
...
#+end_src
*** =cdls=
#+begin_src fish :tangle ~/.config/fish/functions/cdls.fish
...
#+end_src
*** =vils=
#+begin_src fish :tangle ~/.config/fish/functions/vils.fish
...
#+end_src
*** =chbg=
#+begin_src fish :tangle ~/.config/fish/functions/chbg.fish
  function chbg
      set -l path (
          ls 2>/dev/null \
...
#+end_src
**** Completions
#+begin_src fish :tangle ~/.config/fish/completions/chbg.fish
...
#+end_src
*** =vicmd=
#+begin_src fish :tangle ~/.config/fish/functions/vicmd.fish
  function vicmd
      ...
      if [ -f "$file" ]
          ...
          if [ "$choice" = 'y' ]
              smart-launch myemacs -c ~/.haris/scripts.org
          else
              echo 'Aborting...'
              return 1
          end
...
*** =o=
#+NAME: o
#+begin_src fish :tangle ~/.config/fish/functions/o.fish :eval no
  ...
  function o
      ...
      if [ -n "$_flag_h" ]
          ...
          echo "  -t, --title"         \t"Set the tmux session name and treat as a singleton:"
          echo                     \t\t\t"  - if running already, do nothing"
          echo                     \t\t\t"  - if lingering idle, kill it and start fresh"
          return
      end
      ...
      set -l tmux_session
      if [ -n "$_flag_title" ]
          set tmux_session "[o] $_flag_title"
          set -l existing (tmux list-sessions -F '#{session_name}' 2>/dev/null)
          if contains -- "$tmux_session" $existing
              return 0
          end
          if contains -- "$tmux_session [IDLE]" $existing
              tmux kill-session -t "=$tmux_session [IDLE]"
          end
      else
          set tmux_session "[o] $(echo "$__cmdline" | head -c20) [$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | head -c 5)]"
      end
...
#+end_src
**** Completions
#+begin_src fish :tangle ~/.config/fish/completions/o.fish
  complete -c o --wraps command
#+end_src
*** =enved=
#+begin_src fish :tangle ~/.config/fish/functions/enved.fish
...
#+end_src
*** =funcat=
#+begin_src fish :tangle ~/.config/fish/functions/funcat.fish
  function funcat
      type $argv[1] | sed '1,3d;$d' | fish_indent
  end
#+end_src
*** =^=
#+begin_src fish :tangle ~/.config/fish/functions/^.fish
...
#+end_src
**** Completions
#+begin_src fish :tangle ~/.config/fish/completions/^.fish
  complete -c '^' -a '(pwd | tr "/" "\n")' -f
#+end_src
*** =forever=
#+begin_src fish :tangle ~/.config/fish/functions/forever.fish
  function forever
      while :
          $argv
          sleep 2s
...
#+end_src
**** Completions
#+begin_src fish :tangle ~/.config/fish/completions/forever.fish
  complete -c forever --wraps command --wraps builtin
#+end_src
*** =pdm=
#+begin_src fish :tangle ~/.config/fish/functions/pdm.fish
  function pdm
      set cmd $argv[1]
...
#+end_src
*** =tmux=
Inside a =tmux-shell= session (transparent, prefix-disabled, used purely for
scrollback capture / shared access — session names are =term-NNN=), running
real =tmux= would refuse to start because =$TMUX= is set. Clear =$TMUX= in
that case so a real, multiplexing tmux can launch on top.
#+begin_src fish :tangle ~/.config/fish/functions/tmux.fish
  function tmux
      if test -n "$TMUX"
          and string match -q 'term-*' -- (command tmux display-message -p '#{session_name}' 2>/dev/null)
          TMUX='' command tmux $argv
      else
          command tmux $argv
      end
  end
#+end_src
*** =secclip=
#+begin_src fish :tangle ~/.config/fish/functions/secclip.fish
  function secclip --description "Copy text to clipboard for a limited time" --argument-names duration
      set -g content "$(xsel -b -o)"
      if [ -z "$content" ]
          echo "No clipboard content detected. Copy something first and rerun."
          return
      end
      set -g masked "$(echo "$content" | sed -E 's/^(..).*(..)$/\1****\2/')"
      echo "Your secret: $masked"
      set_color --bold red; echo "All occurrences of this secret will be deleted from the clipmenu history when the current shell exits"
      echo "If this is not your intended secret, abort."
      set_color normal

      read -n1 -P "Proceed? (y/N): " answer

      export HARIS_SECCLIP=1
      ...
      function (tmp_func) --on-event fish_exit
          notify-send "secclip" "Deleting secret '$masked' from clipboard history"
          clipdel -d "$content"
      end
  end
#+end_src
*** Worktree utilities
**** =wt=
#+begin_src fish :tangle ~/.config/fish/functions/wt.fish
  function wt --argument-names wt_index subpath
      ...
      if [ -n "$wt_index" ]
          set worktree_root (git worktree list | cut -f1 -d' ' | grep -v '\.claude/worktrees' | grep "wt$wt_index\$")
      else
          set worktree_root (git worktree list | grep -v '\.claude/worktrees' | fzf | cut -f1 -d' ')
      end
...
#+end_src
***** Completions
#+begin_src fish :tangle ~/.config/fish/functions/__haris_complete_wt.fish
  function __haris_complete_wt
      ...
      if [ (count $args) -gt 1 ]
          ...
          set new_git_root (git worktree list | cut -f1 -d' ' | grep -v '\.claude/worktrees' | grep "wt$wt_index\$")
          if [ -z "$new_git_root" ]
              return
          end
      end
...
#+begin_src fish :tangle ~/.config/fish/completions/wt.fish
  complete -c wt -fk -a '(__haris_complete_wt)'
#+end_src
**** =wt_setup_compose=
#+begin_src fish :tangle ~/.config/fish/functions/wt_setup_compose.fish
...
#+begin_src fish
  ...
  function ecf;    set -l file (cf "$argv");   test -e "$file" && smart-launch myemacs -c "$file" ; end

  function cdcmd;  set -l file (fcmd "$argv"); test -f "$file" && cd (dirname "$file"); end
  function catcmd
      set -l file (fcmd "$argv")
      test -f "$file"; or return
      if file --mime "$file" | string match -q '*charset=binary*'
          echo "(binary: $file)"
      else
          cat "$file"
      end
  end
  function ecmd;   set -l file (fcmd "$argv"); test -f "$file" && smart-launch myemacs -c "$file" ; end
  function rmcmd;  set -l file (fcmd "$argv"); rm "$file";                              end
#+end_src
**** =eman=
#+begin_src fish :tangle ~/.config/fish/functions/eman.fish
  function eman
...
#+end_src
**** =tmp_func=
#+begin_src fish :tangle ~/.config/fish/functions/tmp_func.fish
  function tmp_func
      set -g __haris_tmp_func_counter (math (set -q __haris_tmp_func_counter; and echo $__haris_tmp_func_counter; or echo 0) + 1)
      set -g __haris_latest_tmp_func_name "__tmp_func_{$fish_pid}_$__haris_tmp_func_counter"
      echo "$__haris_latest_tmp_func_name"
  end
#+end_src
***** =tmp_func_consume_content=
#+begin_src fish :tangle ~/.config/fish/functions/tmp_func_consume_content.fish
...
#+begin_src fish :tangle ~/.config/fish/functions/no_positional_args.fish
...
#+end_src
*** =snip=
#+begin_src fish :tangle ~/.config/fish/completions/snip.fish
  complete -c snip -f -a \
      "(pushd ~/.vim/snips; command ls | sed 's_\(.*\)\.snippets_\1_g'; popd)"
#+end_src
*** =adhoc=
#+begin_src fish :tangle ~/.config/fish/completions/adhoc.fish
  complete -c adhoc -f \
      -a "(complete -C'adsfadadflasdjflasdflnasdflasdu /tmp/adhoc-files/' | string replace /tmp/adhoc-files/ \"\")"
#+end_src
*** =pass=
#+begin_src fish :tangle ~/.config/fish/completions/pass.fish
...
#+end_src
*** =otp=
#+begin_src fish :tangle ~/.config/fish/completions/otp.fish
    complete -c otp -a '(fd "otp-secret.gpg" ~/.password-store -x echo {//} | sed "s:^.*/\.password-store/\?::")' -f
#+end_src
*** =vscode=
#+begin_src fish :tangle ~/.config/fish/completions/vscode.fish
...
#+end_src
**** =bufls=
#+begin_src fish :tangle ~/.config/fish/completions/bufls.fish
  set -l source "$(bufls completion fish 2>/dev/null)"
  «completions/self-bootstrapping/common»
#+end_src
**** =buf=
#+begin_src fish :tangle ~/.config/fish/completions/buf.fish
  set -l source "$(buf completion fish 2>/dev/null)"
  «completions/self-bootstrapping/common»
#+end_src
**** =gh=
#+begin_src fish :tangle ~/.config/fish/completions/gh.fish
...
#+begin_src fish
  ...
  function vrg --wraps rg; nvim (rg -l $argv); end
  function vfd --wraps fd; nvim (fd $argv); end
#+end_src
**** =dragon=
#+begin_src fish :tangle ~/.config/fish/functions/dragon.fish
...
#+begin_src fish
  function conditional_abbr
      set -l command (string split -m1 ' ' -- $argv[-1])[1]
...
***** Bun
#+begin_src fish
  _abbr -g bi  "export BUN_OPTIONS='--inspect=localhost:6499/%'"
  _abbr -g biw "export BUN_OPTIONS='--inspect-wait=localhost:6499/%'"
  _abbr -g bib "export BUN_OPTIONS='--inspect-brk=localhost:6499/%'"
  _abbr -g bo  "export BUN_OPTIONS='%'"
#+end_src
***** pnpm
#+begin_src fish
  _abbr -g N  'pnpm'
  _abbr -g Nf 'pnpm --filter'
#+end_src
***** OpenCode
#+begin_src fish
  _abbr -g OC    "cd ~/proj/contrib/opencode/packages/opencode"
#+end_src
**** Clipboard
#+begin_src fish
...
#+begin_src fish
  ...
  conditional_abbr -g p1    'ping 1.1.1.1'
  ...
  conditional_abbr -g n     'notify-send'

  conditional_abbr -g tf    'terraform'
  conditional_abbr -g tfw   'terraform workspace'
  conditional_abbr -g tfws  'terraform workspace select'

  # Personal website
  conditional_abbr -g V     'veracioux'
#+end_src
***** Systemd
****** [Guard]
#+begin_src fish
...
#+begin_src fish
  ...
  # New (more brief)
  _abbr -g S         'sudo systemctl'
  _abbr -g Ss        'systemctl status'
  _abbr -g SS        'sudo systemctl start'
  _abbr -g S.        'sudo systemctl stop'
  _abbr -g Se        'sudo systemctl enable'
  _abbr -g Sd        'sudo systemctl disable'
  _abbr -g Sdr       'sudo systemctl daemon-reload'
  _abbr -g Sr        'sudo systemctl reload'
  _abbr -g SR        'sudo systemctl restart'
  _abbr -g Sdrr      'sudo systemctl daemon-reload --user && systemctl restart  --user'
  _abbr -g Sc        'systemctl cat'
  _abbr -g Sl        'systemctl list-units'
...
#+begin_src fish
  ...
  # New (more brief)
  _abbr -g s         'systemctl --user'
  _abbr -g ss        'systemctl status --user'
  _abbr -g sS        'systemctl start --user'
  _abbr -g s.        'systemctl stop --user'
  _abbr -g se        'systemctl enable --user'
  _abbr -g sd        'systemctl disable --user'
  _abbr -g sdr       'systemctl daemon-reload --user'
  _abbr -g sr        'systemctl reload --user'
  _abbr -g sR        'systemctl restart  --user'
  _abbr -g sdrr      'systemctl daemon-reload --user && systemctl restart  --user'
  _abbr -g sc        'systemctl --user cat'
  _abbr -g sl        'systemctl --user list-units'
...
#+begin_src fish
  ...
  conditional_abbr -g C     'claude'
  conditional_abbr -g CR    'claude --resume'
  conditional_abbr -g oc    'opencode'
  conditional_abbr -g oc1   'bunx opencode-ai@latest'
  conditional_abbr -g oct   'octave'
  conditional_abbr -g yt    'ytfzf -t -s'
  conditional_abbr -g y     'yt-dlp'
  ...
  conditional_abbr -g h     'hostess'
  conditional_abbr -g cat   'bat'
#+end_src
**** Utils
#+begin_src fish
  if [ "$TERM" != "linux" ]
...
**** Google cloud
#+begin_src fish
  conditional_abbr -g gc  'gcloud compute'
  conditional_abbr -g gdc 'gcloud compute disks create --image=%'
  conditional_abbr -g gdd 'gcloud compute disks describe'
  conditional_abbr -g gd  'gcloud compute disks'
  conditional_abbr -g gdl 'gcloud compute disks list'
  conditional_abbr -g gia 'gcloud compute instances attach-disk --disk=%'
  conditional_abbr -g gIc 'gcloud compute images create --source-disk=%'
  conditional_abbr -g gic 'gcloud compute instances create'
  conditional_abbr -g gid 'gcloud compute instances describe'
  conditional_abbr -g giD 'gcloud compute instances detach-disk --disk=%'
  conditional_abbr -g gI  'gcloud compute images'
  conditional_abbr -g gi  'gcloud compute instances'
  conditional_abbr -g gil 'gcloud compute instances list'
  conditional_abbr -g gis 'gcloud compute instances start'
  conditional_abbr -g giS 'gcloud compute instances stop'
  conditional_abbr -g gsc 'gcloud compute snapshots create --source-disk=%'
  conditional_abbr -g gs  'gcloud compute snapshots'
  conditional_abbr -g gsl 'gcloud compute snapshots list'
#+end_src
**** Tmux
#+begin_src fish
  # Tmux
  ...
  if type --query tmp_func
      ...
      conditional_abbr -g xa   "$(tmp_func_consume_content)"
  end
#+end_src
**** VirtualBox
#+begin_src fish
...
** Helper functions
#+begin_src shell
  # Export a variable in shell and register to add in as well ~/.config/environment.d/
  export_both() {
      if [ "$#" -gt 1 ]; then
          echo "Usage: export_both VARIABLE=VALUE (only one argument allowed)"
          return 1
      fi
      export "$1"
      __haris_exported_vars="$__haris_exported_vars"$'\n'"$1"
  }
  __haris_exported_vars=""

  # Write registered variables to ~/.config/environment.d/20-from-shell.conf
  flush_exports() {
      conf_dir="$HOME/.config/environment.d"
      conf_file="$conf_dir/20-from-shell.conf"

      # Read existing file into variable
      local existing_content=""
      if [ -f "$conf_file" ]; then
          existing_content="$(cat "$conf_file")"
      fi

      # Compare with $__haris_exported_vars
      if [ "$existing_content" != "$__haris_exported_vars" ]; then
          echo "Environment variables updated. Please re-login for changes to take effect in systemd services." >&2

          mkdir -p "$conf_dir"

          echo "$__haris_exported_vars" > "$conf_file"
      fi
  }
#+end_src
** Executable paths
#+begin_src shell
  mkdir -p ~/.go
  export_both GOPATH="$HOME"/.go

  # Expand the PATH variable. Each added entry is in ascending precedence
  export PATH=/usr/local/bin:"$PATH"
  if [ -n "$CLOUDSDK_ROOT_DIR" ]; then
      export PATH="$CLOUDSDK_ROOT_DIR"/bin:"$PATH"
  fi
  export PATH=~/.go/bin:"$PATH"
  gempath="$(command -v gem >/dev/null && gem environment gempath)"
  if [ -n "$gempath" ]; then
...
** Miscellaneous
#+begin_src shell
  export_both COMPOSE_BAKE=true
  export_both MAKEFLAGS='-j6'
  export_both GPG_TTY=$(tty)
  export_both PYTHONSTARTUP=~/.startup.py
  export_both RUSTC_WRAPPER=sccache
  export_both MOZ_USE_XINPUT2=1
  # Clipmenu
  # (ref:CM_DIR)
  export_both CM_DIR="$HOME/.local/share/haris/clipmenu"
  export_both RCLONE_PASSWORD_COMMAND="pass rclone/config"

  # Set up vagrant home for reuse
  VAGRANT_HOME=/usr/local/share/"$USER"/vagrant
  if [ -d "$VAGRANT_HOME" ]; then
      export_both VAGRANT_HOME="$VAGRANT_HOME"
  fi

  [ "$(get-os-type)" = "arch" ] && export_both QT_QPA_PLATFORMTHEME=gtk2

  export_both PNPM_HOME=$HOME/.local/share/pnpm

  # If rootless docker is used, configure the DOCKER_HOST variable
  if systemctl is-enabled --user --quiet docker.service; then
      export_both DOCKER_HOST=unix://$XDG_RUNTIME_DIR/docker.sock
  fi

  # Better pager for psql command
  if which pspg >/dev/null 2>&1; then
      # '-s 5' chooses the mutt theme
      export_both PSQL_PAGER='pspg -s 5'
  fi
...
#+begin_src shell
  export_both PATH="$(
      ...
      printf "%s" "$PATH" | awk -v RS=: '!seen[$0]++' | paste -sd:
  )"
  flush_exports
  # Sentinel so fish can detect whether the wrapping shell already sourced this
  export HARIS_PROFILE_LOADED=1
#+end_src
