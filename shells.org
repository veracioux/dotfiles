...
#+begin_src fish :tangle (haris/tangle-home ".config/fish/completions/forever.fish")
  complete -c forever -a '(complete -C "")'
#+end_src
...
***** =conditional_abbr=
#+begin_src fish
  # An abbreviation of a command name only, with no CLI arguments
  function conditional_abbr
      set -l command (echo $argv[-1] | cut -f1 -d' ')
      if command --quiet "$command"
          abbr -g $argv
      end
  end
#+end_src
...
#+end_src
#+begin_src fish
  if command --quiet pacman || which --quiet apt
#+end_src
***** Abbreviations
#+begin_src fish
...
**** Docker
#+begin_src fish
  conditional_abbr -g d      'docker'
  conditional_abbr -g db     'docker build'
  conditional_abbr -g dbn    'docker build --no-cache'

  conditional_abbr -g dr     'docker run'
  conditional_abbr -g drit   'docker run -it'
  conditional_abbr -g drite  'docker run -it --entrypoint'

  conditional_abbr -g deit   'docker exec -it'

  conditional_abbr -g drm    'docker rm -f'

  conditional_abbr -g di     'docker image'
  conditional_abbr -g dip    'docker image prune'
  conditional_abbr -g dii    'docker image inspect'

  conditional_abbr -g dc     'docker container'
  conditional_abbr -g dcl    'docker container ls'
  conditional_abbr -g dcp    'docker container prune'
  conditional_abbr -g dci    'docker container inspect'

  conditional_abbr -g dv     'docker volume'
  conditional_abbr -g dvl    'docker volume ls'
  conditional_abbr -g dvi    'docker volume inspect'
  conditional_abbr -g dvp    'docker volume prune'

  conditional_abbr -g ds     'docker system'
  conditional_abbr -g dsp    'docker system prune'
  conditional_abbr -g dsd    'docker system df -v'

  conditional_abbr -g dm     'docker manifest'
  conditional_abbr -g dmi    'docker manifest inspect'

  conditional_abbr -g dhh    'docker history'
#+end_src
***** Compose
#+begin_src fish
  ...
  conditional_abbr -g Drm    'docker-compose rm -f'
...
#+begin_src fish
  ...
  conditional_abbr -g w     'watch -n0.1'
