...
#+begin_src fish :tangle (haris/tangle-home ".config/fish/functions/fish_right_prompt.fish")
  function fish_right_prompt
      ...
      if xdotool search --all --onlyvisible "" 2>/dev/null | ! grep -q "$WINDOWID"
          ...
          timeout 0.8 notify-send (echo "Command "(history | tac | tail -n +1 | tail -n +"$__haris_history_count" 2>/dev/null | head -1)"" "$exit_message")
          if [ "$status" = "124" ]
...
#+begin_src fish :tangle (haris/tangle-home ".config/fish/functions/o.fish") :eval no
  ...
  function o
      ...
      tmux new -d \
          -s "$(echo "$__cmdline" | head -c20) [$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | head -c 5)]" \
...
#+begin_src fish
  ...
  # Tmux
  abbr -g x   'tmux'
  abbr -g X   'tmux attach -t (tmux list-sessions -F "#{session_created} #{session_name}" | sort | tac | grep -Po "(?<= ).*" | fzf -s)'
#+end_src
*** Variables
#+begin_src fish
