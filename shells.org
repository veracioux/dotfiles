...
#+begin_src fish :tangle (haris/tangle-home ".config/fish/functions/wt_setup_compose.fish")
  function wt_setup_compose --argument-names project worktree_index
      ...
      set -gx EXPOSED_IP (dig +short "$COMPOSE_PROJECT_NAME")
      ...
      if [ -z "$EXPOSED_IP" ]
          ...
          set_color normal
          echo
          export EXPOSED_IP=(dig +short localhost)
      end
...
#+begin_src fish
  if [ "$__haris_docker" = "docker" ] &&
      ...
      # Restart using rm + up
      set -l tmp_func_name __tmp_(random)
      function "$tmp_func_name"
          begin
              set -l container %
      ...
      _abbr -g DRR    "$(funcat "$tmp_func_name")"
      functions --erase "$tmp_func_name"
  end
#+end_src
**** Pass
#+begin_src fish
