...
#+begin_src fish :tangle ~/.config/fish/functions/__haris_prompt_env_vars.fish
  function __haris_prompt_env_vars --argument-names cmd
      ...
      ' "$file" (env | psub) | while read -l line
          ...
          switch "$var"
              case WINDOWID RCLONE_PASSWORD_COMMAND ALACRITTY_SOCKET \
                  DEBUGINFOD_URLS ALACRITTY_WINDOW_ID ALACRITTY_LOG SHELL \
                  LSP_USE_PLISTS INVOCATION_ID MEMORY_PRESSURE_WATCH \
                  MEMORY_PRESSURE_WRITE PWD SYSTEMD_EXEC_PID MANAGERPID \
                  MANAGERPIDFDID JOURNAL_STREAM WORKTREE EXPOSED_IP \
                  COMPOSE_PROJECT_NAME SHLVL TMUX_PANE NO_AT_BRIDGE TERM_PROGRAM \
                  VSCODE_GIT_ASKPASS_MAIN VSCODE_GIT_ASKPASS_NODE CHROME_DESKTOP \
                  GDK_BACKEND PYDEVD_DISABLE_FILE_VALIDATION TERM_PROGRAM_VERSION \
                  VSCODE_DEBUGPY_ADAPTER_ENDPOINTS PYTHON_BASIC_REPL VSCODE_NONCE \
                  GIT_ASKPASS ORIGINAL_XDG_CURRENT_DESKTOP VSCODE_GIT_IPC_HANDLE \
                  BUNDLED_DEBUGPY_PATH HARIS_CALLER_IS_DMENU EMACS_VTERM_PATH \
                  INSIDE_EMACS STY TERMCAP SXHKD_PID I3SOCK DESKTOP_SESSION WINDOW \
                  GTK_MODULES GSM_SKIP_SSH_AGENT_WORKAROUND NIX_REMOTE \
                  QT_ACCESSIBILITY CREDENTIALS_DIRECTORY \
                  DOCKER_CLIENT_TIMEOUT COMPOSE_HTTP_TIMEOUT PGPASSWORD
                  continue
              case NODE_ENV
                  if [ -z "$runtime_value" ]
                      continue
