...
#+end_src
** Private
#+transclude: [[file:./private/README.org::fish-private][fish-private]] 
* Bash
...
#+begin_src shell :tangle (concat "/tmp/tangle-" (user-login-name) "/.alias")
  ...
  alias src='cd ~/src'
  alias tmp='cd /tmp'
  alias stage='mkdir -p /tmp/stage; chmod o+w /tmp/stage; cd /tmp/stage'
...
#+begin_src fish
  ...
  abbr -g sprq  'cmd-with-notify sudo pacman -R (pacman -Qdtq)'
  ...
  abbr -g dis       'systemctl disable'
  abbr -g sts       'systemctl status'
  abbr -g dreload   'systemctl daemon-reload'
  abbr -g reload    'systemctl reload'
  ...
  abbr -g dreloadu  'systemctl daemon-reload --user'
  abbr -g reloadu   'systemctl reload --user'
