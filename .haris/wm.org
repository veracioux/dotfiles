...
#+PROPERTY: header-args :tangle (haris/tangle-home ".config/i3/config") :mkdirp yes :noweb yes
...
#+begin_src i3wm-config
  ...
  set $mode_org     " ORG [t]odo [k]knowledge [j]ournal [n]otes [c]omputers [l]ifestyle c[z]ech"
  set $mode_mic     " Microphone [+] [-] [m]ute"
  set $mode_emacs   " Emacs Goodies | [e]lisp [p]ython [d]ocker [i]rc [g]it [o]ctave [⏎]term"
  set $mode_trans   " Translate to | [e]nglish [c]zech [b]osnian [g]erman"
#+end_src
...
#+begin_src shell :tangle (haris/tangle-home ".xinitrc-i3")
  ...
  dbus-update-activation-environment \        # Fixes dbus problems with i3
...
#+begin_src shell :tangle (haris/tangle-home ".xinitrc.common")
  system-specific                             # System specific config
...
swhkd on Wayland. See [[*Startup][Startup]].
#+begin_src i3wm-config
...
#+begin_src i3wm-config
  ...
  mode default
  bindsym z               exec --no-startup-id "myemacs-float ~/wiki/czech.org"; \
  mode default
...
#+begin_src i3wm-config
  ...
  bindsym $mod+Shift+t mode $mode_trans
#+end_src
* Workspaces
#+NAME: workspaces
#+begin_src i3wm-config
