#+TITLE: Emacs Config
#+PROPERTY: header-args :tangle (haris/tangle-home ".emacs.d/haris/default.el") :mkdirp yes :noweb yes
...
with many sensible defaults and simplifies the installation and setup of packages.
Because it is so feature-rich, Emacs can take an annoying time to start up.
...
command that wraps [[man:emacsclient][emacsclient]]), which uses the daemon, without the need to load
the cruft each time.
...
#+begin_src emacs-lisp
  ...
  (defun haris/search-in-files-with-initial ()
    ...
    (interactive)
    (if (eq evil-state 'visual) (counsel-rg (projectile-symbol-or-selection-at-point)) (counsel-rg)))
...
#+begin_src emacs-lisp
  ...
  (use-package ol-man :ensure nil :defer t)
        ...
        (cl-remove-duplicates
         (append (default-value 'org-structure-template-alist)
                 '(("el"  . "src emacs-lisp")
                   ...
                   ("elt" . "src emacs-lisp :tangle (haris/tangle-home \"TODO\")")))
...
those files to my live config under =~/=, I can call =(haris/tangle)=.
#+begin_src emacs-lisp
...
#+end_src
The following functions help me define common tangle destinations.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp :tangle (haris/tangle-home ".emacs.d/haris/irc.el")
  ...
  (define-key       erc-list-menu-mode-map  (kbd "RET")   'erc-list-join)
#+end_src
** Theme tweak
#+begin_src emacs-lisp :tangle (haris/tangle-home ".emacs.d/haris/irc.el")
