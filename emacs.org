...
#+begin_src elisp
  (eval-after-load 'dap-mode
    (lambda ()
      ...
      (add-hook 'dap-terminated-hook 'normal-mode 99)
      (setq dap-ui-inspect-mode-hook (delq 'evilified-state dap-ui-inspect-mode-hook))))
#+end_src
...
#+begin_src elisp
  ...
  ;; A single company-capf item in the company-backends list breaks snippets
  ;; Replace it with (company-capf :with company-yasnippet)
...
#+begin_src elisp :tangle yes
  (defalias 'dap-python--pyenv-executable-find 'executable-find)
#+end_src
*** Bindings
#+begin_src elisp
  (eval-after-load 'lsp-mode
...
# Disabled posframe in [[(disable-posframe)][Spacemacs Layers]].
** Yasnippet
#+begin_src elisp
