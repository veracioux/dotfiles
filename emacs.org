...
#+begin_src bash
  ...
  if [ ! -d .git ]; then
      ...
      git checkout «spacemacs-good-commit»
  fi
  popd >/dev/null
#+end_src
#+NAME: spacemacs-good-commit
#+begin_src text
  4a227fc94651136a8de54bcafa7d22abe1fa0295
#+end_src
*** Interactively install spacemacs
#+begin_src bash
  ln -s ~/.haris/.spacemacs ~/.spacemacs
  echo "Emacs will be launched now. Follow the prompts to setup spacemacs"
  echo "When done, you can close the emacs window"
  COLORTERM=truecolor LSP_USE_PLISTS=true emacs
#+end_src
*** Install themes
#+begin_src bash
  themes_dir=~/.emacs.d/private/themes

  mkdir -p "$themes_dir/dracula"

  git clone 'https://github.com/veracioux/dracula-emacs' "$themes_dir/dracula"

  cd "$themes_dir"
  ln -sf dracula/dracula-theme.el dracula-theme.el
#+end_src
** Theme
...
#+begin_src elisp
  ...
  ;; Next language
  (evil-define-key 'normal gt-buffer-prompt-map (kbd "M-n") (kbd "C-c C-n"))
  ;; Previous language
  (evil-define-key 'normal gt-buffer-prompt-map (kbd "M-p") (kbd "C-c C-p"))
  ...
  (define-key chatgpt-mode-map (kbd "RET")
              (defun haris//chatgpt-type-response ()
                ...
                (let ((haris/chatgpt-session-buffer (current-buffer)))
                  (chatgpt-type-response))))
  ;; CodeGPT mode
  (evil-define-key 'insert codegpt-mode-map (kbd "q") 'quit-window)
  (evil-define-key 'normal codegpt-mode-map (kbd "q") 'quit-window)
