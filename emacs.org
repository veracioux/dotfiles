...
#+begin_src elisp
  ...
  (straight-use-package '(use-package :source melpa))
...
#+begin_src elisp
  ...
  ;; Mark my own dotfiles as safe for all local variables
  (defun haris/advice/whitelist-local-variables-in-trusted-files (orig-fun &rest args)
    (let ((file-name ""))
      ;; Get the file-name the operation applies to
      (if (eql (type-of (car args)) 'string)
          (setq file-name (car args))
        (setq file-name buffer-file-name))
      ;; If file whitelisted, enable local variables and local eval
      (if (string-prefix-p (expand-file-name "~/.haris") file-name)
          (let ((enable-local-variables :all)
                (enable-local-eval t))
            (apply orig-fun args))
        (apply orig-fun args))))

  ;; Whitelist local variables and evals in some files I trust
  (advice-add #'find-file-noselect :around #'haris/advice/whitelist-local-variables-in-trusted-files)
  (advice-add #'revert-buffer      :around #'haris/advice/whitelist-local-variables-in-trusted-files)
...
*** Fish
#+begin_src elisp
  (straight-use-package '(company-fish :type git :host github :repo "CeleritasCelery/company-fish"))
#+end_src
** RST
#+begin_src elisp
...
* Minibuffer
#+begin_src elisp
  (add-hook 'minibuffer-setup-hook #'haris/yas-minor-mode-on 90)
  (add-hook 'minibuffer-setup-hook
            (defun haris//company-setup-in-minibuffer ()
...
#+begin_src elisp
  ...
  (evil-define-key 'insert ivy-minibuffer-map (kbd "<backtab>")
    (lambda () (interactive) (ivy-insert-current)))
#+end_src
#+begin_src elisp :tangle (haris/tangle-home ".emacs.d/haris/irc.el") :exports none
  <<ivy>>
#+end_src
...
#+begin_src elisp
  ...
  (setq vterm-min-window-width 99999)
...
#+begin_src elisp
  (use-package company)
  (straight-use-package '(company-statistics :source melpa))
...
#+begin_src elisp
  ...
  (defun haris/init-company-for-mode (mode)
...
#+begin_src elisp
  (setq haris/company-backends-sh-mode
        '(
  ...
  (haris/init-company-for-mode 'sh-mode)

  (remove-hook 'org-mode-hook #'spacemacs//init-company-org-mode)
  (setq haris/company-backends-org-mode
        '(haris/company-tempo
          (company-emoji :with company-yasnippet)
          ...
          (company-files :with company-yasnippet)
          (company-dabbrev :with company-yasnippet)))
  (haris/init-company-for-mode 'org-mode)

  (setq haris/company-backends-nginx-mode
        '((company-emoji :with company-yasnippet)
          (company-nginx company-semantic company-dabbrev-code company-gtags company-etags company-keywords :with company-yasnippet)
          (company-files :with company-yasnippet)
          (company-dabbrev :with company-yasnippet)))
  (haris/init-company-for-mode 'nginx-mode)
#+end_src
...
#+begin_src elisp
  ...
  (define-key company-active-map (kbd "<escape>") #'haris/company-toggle-tooltip)
#+end_src
** Yasnippet
#+begin_src elisp
  ...
  (add-hook 'git-commit-mode-hook #'haris/yas-minor-mode-on)
  (add-hook 'git-commit-mode-hook (lambda () (yas-activate-extra-mode 'git-commit-mode)))
  ...
  (add-hook 'yas-before-expand-snippet-hook #'haris/company-disable-idle-popup)
  (add-hook 'yas-after-exit-snippet-hook #'haris/company-enable-idle-popup)
#+end_src
** The =TAB= key
...
** Daemons
#+begin_src elisp
  (straight-use-package '(daemons :source melpa))
...
** Nginx
#+begin_src elisp
  (straight-use-package '(company-nginx :source melpa))
#+end_src
* Spacemacs
#+begin_src elisp
