...
#+begin_src elisp
  ...
  (add-hook 'git-commit-mode-hook (lambda () (set (make-local-variable 'yas-extra-modes) '(git-commit-mode))))
  (add-hook 'verb-mode-hook (lambda () (set (make-local-variable 'yas-extra-modes) '(verb-mode))))
  ...
  (defun haris/handle-TAB-in-insert-mode (&optional alert)
    ...
    (let ((last-fun nil))
      ...
      (cond
       ...
       ((when yas-minor-mode (ignore-errors (or (_ 'yas-next-field) t))))
       ;; Company/yasnippet completion
       ((when (> (length (company-grab-symbol)) 0)
          (cond
           ;; If yas-minor-mode is on, try yas-next-field, otherwise yas-expand
           ((when yas-minor-mode (_ 'yas-expand)))
       ...
       ((ignore-errors (_ 'call-interactively #'company-capf)))
       ;; Fallback
