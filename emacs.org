...
** Eamcs Daemon
#+NAME: emacs.service
#+begin_src systemd :tangle ~/.config/systemd/user/emacs.service :exports none
  [Unit]
...
#+begin_src elisp
  ...
  (defun haris/projectile-open-in-vscode ()
    (interactive)
      (haris/vscode (projectile-project-root)))
...
#+begin_src elisp
  (with-eval-after-load 'lsp-mode
    ...
    ;; Reduce lsp-ui-sideline frequency to improve performance
...
#+end_src
**** PET Mode
PET mode is unbearably slow.
#+begin_src elisp
  (add-hook 'prog-mode-hook
            (lambda ()
              (remove-hook 'python-base-mode-hook 'pet-mode)))
#+end_src
*** Bindings
#+begin_src elisp
  (with-eval-after-load 'lsp-mode
...
#+end_src
By default, python-pytest-mode activates only =compilation-minor-mode=, but I also
want bindings (link following etc) from =compilation-mode-map= to be available.
#+begin_src elisp
  (add-hook 'python-pytest-finished-hook
            (lambda () (use-local-map compilation-mode-map)))
#+end_src
*** Dependencies                                                     :deps:
...
*** Unit Testing
#+begin_src elisp
  (use-package jest-test-mode :ensure t)
...
#+begin_src elisp
  (straight-use-package
   '(scss-mode :type git :host github :repo "veracioux/scss-mode"))
#+end_src
** Vue
...
** CircleCI
#+begin_src elisp
  (defcustom haris/lsp-circleci-schema-path
    "UNDEFINED"
    "The schema.json path for circleci-yaml-language-server"
    :type '(string))
     ...
     (make-lsp-client
      ...
      (lsp-stdio-connection
       `("circleci-yaml-language-server"
         "-stdio"
         "-schema"
         ,haris/lsp-circleci-schema-path))
      :activation-fn (lsp-activate-on "yaml")
      :server-id 'circleci-ls)))
#+end_src
** SQL
#+begin_src elisp
...
#+begin_src elisp
  ...
  (add-to-list 'ivy-sort-matches-functions-alist
               `(counsel-projectile-switch-project
                 .
                 ,(defun haris//ivy-sort-alpha (_ candidates)
...
#+end_src
*** Custom actions
I extend the Ivy actions for some commands.
#+begin_src elisp
       ...
       ("\""
        (lambda (project)
          (let ((default-directory project))
            (terminal-here)))
        "open terminal in project")
       ;; Prompts for a command, then runs multi-vterm-project and the command in it
       ("!"  ;; prompt for a cmd, then open vterm + run it
        (lambda (project)
          (let* ((default-directory project)
                 (cmd (read-shell-command "Run in vterm: ")))
            (multi-vterm-project)
            (vterm-send-string cmd)
            (vterm-send-return)))
        "run a command in project vterm")
       ("a" ;; "a" stands for AI
        (lambda (project)
          (let ((default-directory project))
            (terminal-here '("fish" "-C" "opencode"))))
        "open opencode in project")
       ("V"
        (lambda (project)
          (start-process "vscode" nil "o" "vscode" "--new-window" (expand-file-name project)))
...
#+begin_src elisp
  ...
  (defun haris/org-babel-goto-tangle-file ()
    ...
    (let ((file (haris/extract-tangle-dest)))
        ...
        (cl-remove-duplicates
         (append (default-value 'org-structure-template-alist)
                 '(("el"        . "src elisp")
                   ...
                   ("st"        . "src shell :tangle (haris/tangle-home \"TODO\")")
                   ("et"        . "src elisp :tangle (haris/tangle-home \"TODO\")")
...
#+begin_src elisp
  ...
  (with-eval-after-load 'magit-gptcommit
    (with-eval-after-load 'haris/ai
      (setq magit-gptcommit-llm-provider (eval haris/ai/model/claude/sonnet-4.5/creator))
      (setq haris/commit-prompt-default
            (concat
             "THE FILE DIFFS:\n```\n%s\n```\n\n"
             (with-temp-buffer
              (insert-file-contents (expand-file-name "~/.ai/prompts/git-commit.md"))
              (buffer-string))))
      ...
      (setq haris/commit-prompt-dotfiles
            (concat
             "THE FILE DIFFS:\n```\n%s\n```\n\n"
             (with-temp-buffer
              (insert-file-contents (expand-file-name "~/.ai/prompts/git-commit-dotfiles.md"))
              (buffer-string))))

      (setq magit-gptcommit-prompt haris/commit-prompt-default)

      (with-eval-after-load 'magit-status
        (when (not (boundp 'haris//gptcommit-status-buffer-setup-p))
          (defvar haris//gptcommit-status-buffer-setup-p t
...
* AI
#+begin_src elisp
  (with-eval-after-load 'haris/api-keys
    (require 'llm-claude)
    ...
    (setq haris/ai/model/claude/sonnet-4.5/creator
          '(make-llm-claude :key anthropic-key :chat-model "claude-sonnet-4-5-20250929"))
...
*** Bindings
#+begin_src elisp
  (spacemacs/set-leader-keys "#" 'claude-code-transient)
#+end_src
** Gptel
#+begin_src elisp
  (with-eval-after-load 'haris/api-keys
...
#+begin_src elisp
  (with-eval-after-load 'haris/ai
    ...
    (setopt ellama-provider (eval haris/ai/model/openai/o4-mini/creator)))
#+end_src
* REPLs in Emacs
** Common settings
#+begin_src elisp
...
#+begin_src elisp
  ...
  ;; Disable performance-impacting modes in vterm
  ...
  (setq
   ...
   (let* ((common '(company-files
          ...
          (web-mode '(company-files
                      ...
                      company-keywords))
          (org-mode '(company-files
                      company-emoji
                      (company-capf company-org-block :with company-yasnippet)
                      company-keywords
                      company-dabbrev)))
     `((text-mode . ,common)
       (emacs-lisp-mode . ,common)
       (typescript-mode . ,common)
       (python-mode . ,common)
       (sh-mode . ,common)
       (yaml-mode . ,common)
       (yaml-ts-mode . ,common)
       (sql-interactive-mode . ,common)
       (sql-mode . ,common)
       (json-mode . ,common)
       (jsonc-mode . ,common)
       (dockerfile-mode . ,common)
       (web-mode . ,web-mode)
       (css-mode . ,web-mode)
       (scss-mode . ,web-mode)
       (org-mode . ,org-mode))))
...
#+begin_src elisp :eval no
  (defvar haris//original-spacemacs-company-backends-alist nil
    "Default company backends per mode as defined by spacemacs before I overrode them")
#+end_src
#+begin_src elisp
...
** Layers
#+begin_src elisp :tangle ~/.spacemacs-init.el
  (setq-default
   ...
   '(
     ...
     (auto-completion ; (ref:auto-completion)
      :enabled-for  ;; Configured manually for specific languages
      :variables
...
* Appendix
** Config check
I use this variable (and function) to check if the config loaded correctly.
#+NAME: haris/config-loaded-fine
#+begin_src elisp :tangle ~/.emacs.d/haris/root.el
  (setq haris/config-loaded-fine (current-time-string))
  (defun ---haris/config-loaded-fine ()
    (interactive)
    (describe-variable 'haris/config-loaded-fine))
#+end_src
