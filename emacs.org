...
#+begin_src elisp
  (with-eval-after-load 'lsp-mode
    ...
    ;; Disable filesystem watching; rely on buffer-driven didChange events.
    ;; External edits (branch switches, codegen) will require lsp-workspace-restart.
...
#+begin_src elisp
  (transient-append-suffix 'magit-commit "c"
    '(1 "G" "generate (Claude)" (lambda ()
       (interactive)
       (let ((default-directory (magit-toplevel)))
         (start-process "claude-commit" nil "smart-launch" "term" "claude" "--effort" "low" "--model" "sonnet" "/commit")))))
#+end_src
** Repo list
#+begin_src elisp
...
#+end_src
** Emacs MCP Server
Exposes Emacs to MCP clients (e.g. Claude) via a Unix socket. Connect a client
with:
#+begin_example
claude mcp add emacs -- socat - UNIX-CONNECT:$XDG_RUNTIME_DIR/emacs-mcp-server.sock
#+end_example
The =socat= binary must be installed on the system.
#+begin_src elisp
  (use-package mcp-server
    ...
    (setq mcp-server-emacs-tools-enabled 'all
          mcp-server-socket-directory (or (getenv "XDG_RUNTIME_DIR")
