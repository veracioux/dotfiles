...
#+end_src
**** Company backends
=lsp-completion-mode= adds =company-capf= in front of the backends
#+begin_src elisp
  ;; Replace company-capf with (company-capf :with company-yasnippet)
  (spacemacs|add-company-backends
    :backends (company-capf :with company-yasnippet)
    :modes python-mode)
  (setq company-backends-python-mode (cl-delete 'company-capf company-backends-python-mode))

  ;; Revert the mess created by the lsp-completion-mode function on each revert-buffer
  (defun haris//python-fix-company-backends ()
    (setq company-backends company-backends-python-mode))
  (add-hook 'lsp-completion-mode-hook 'haris//python-fix-company-backends)
#+end_src
*** Bindings
#+begin_src elisp
  (eval-after-load 'lsp-mode
...
#+begin_src elisp
  ...
  (defun haris/mic-set-state (state)
    "Set microphone to given STATE: 'muted' or 'unmuted'."
    (shell-command
     (format "pactl set-source-%s @DEFAULT_SOURCE@ 1"
             (if (eq state 'muted)
                 "mute"
               "unmute"))))

  (defun haris/mic-get-state ()
    ;; Implement
    (let ((state (shell-command-to-string "pactl get-source-mute @DEFAULT_SOURCE@")))
      (if (string-match "yes" state)
          'muted
        'unmuted)))

  ;; Add advice around aidermacs-send-voice, making it unmute mic using pactl before and restore after
  (defadvice aidermacs-send-voice (before mic-unmute activate)
    (setq haris//previous-mic-state (haris/mic-get-state))
    (mic-unmute)
    (add-hook 'aidermacs-after-send-hook #'mic-mute 'append))


  (defadvice vterm-send-return (after mic-unmute activate)
    (setq haris//previous-mic-state (haris/mic-get-state))
    (mic-unmute)
    (add-hook 'aidermacs-after-send-hook #'mic-mute 'append))
#+end_src
