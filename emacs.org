          ...
          `(defun ,(intern (format "haris/script/%s" command))
             ...
             (let ((command ,command)
               ...
               (with-current-buffer _vterm
                 ...
                 (rename-buffer (format "*%s*" command))
                 (setq run-command
...
#+begin_src emacs-lisp
  ...
  (spacemacs/set-leader-keys "oc" 'clone-buffer)
...
#+begin_src elisp
  ...
  (defun haris/org-mode-TAB ()
    (interactive)
    (cond ((org-tempo-complete-tag))
          ((org-cycle))))

  (evil-define-key 'insert org-mode-map (kbd "TAB") #'haris/org-mode-TAB)
...
#+end_src
* TODO Completion
I should probably have a single global binding for TAB. In this function, I
can define the precise order of precedence.

Some notes:

- functions that have no other bindings except TAB should have highest precedence
- Should remove TAB (and <tab>) bindings from all other modes
#+begin_src elisp
  ...
  (defun haris/yas-next-field-or-expand ()
    (interactive)
    (cond
     ;; Give org-cycle precedence
     ((and (fboundp 'org-at-table-p) (org-at-table-p)) (org-cycle))
     ((condition-case err
        (yas-next-field)
      (error (yas-expand))))))

  (defun haris/company-complete-selection-prefer-yas-expand ()
    (interactive)
    (cond
     ;; Give org-cycle precedence
     ((and (fboundp 'org-at-table-p) (org-at-table-p)) (org-cycle))
  ...
  ;; Would prefer this binding would take precedence over everything, but it doesn't
  ;; over company completion
  (evil-define-key 'insert yas-minor-mode-map (kbd "TAB")
    '(menu-item "" haris/yas-next-field-or-expand :filter yas-maybe-expand-abbrev-key-filter))

  ;; company-active-map is too invasive
  (define-key company-active-map (kbd "TAB") nil)
  ;; Restrict company-mode TAB to insert mode, just in case
  (define-key company-mode-map (kbd "TAB") nil)
  ;; and also give yas-expand precedence
  (evil-define-key 'insert company-mode-map (kbd "TAB") #'haris/company-complete-selection-prefer-yas-expand)

  (define-key yas-keymap (kbd "TAB") 'yas-next-field)
  (define-key yas-keymap (kbd "<backtab>") 'yas-prev-field)

  ;; These are duplicates of TAB and can cause confusion
  (define-key company-active-map (kbd "<tab>") nil)
...
#+end_src
Note: Some additional variables are configured in [[*Layers][Spacemacs layers]].
* Info mode
...
#+begin_src elisp
  (defun haris/command-log ()
    (interactive)
    (command-log-mode 'toggle)
    (clm/toggle-command-log-buffer))

  (spacemacs/set-leader-keys (kbd "oC") #'haris/command-log)
#+end_src
** Nerd commenter
#+begin_src emacs-lisp
