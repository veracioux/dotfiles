...
This code block wraps elisp's =eval= function so it can be called using noweb. It
evaluates the symbol specified as its =symbol= argument.
#+NAME: eval
#+begin_src emacs-lisp :var symbol="nil" :exports none :tangle no
  (eval (car (read-from-string symbol)))
#+end_src
# Test: must output path to mic-mute-toggle script
#+CALL: eval(symbol="mic-mute-toggle-filename") :results silent
...
#+begin_src i3wm-config :noweb yes
  ...
  mode $mode_mic {
  bindsym plus            exec --no-startup-id '<<eval(symbol="mic-volume-up-filename")>>'
  bindsym minus           exec --no-startup-id '<<eval(symbol="mic-volume-down-filename")>>'
  bindsym m               exec --no-startup-id '<<eval(symbol="mic-mute-toggle-filename")>>'
...
# Local Variables:
# eval: (setq-local scriptdir (haris/tangle-home ".config/i3/scripts/"))
